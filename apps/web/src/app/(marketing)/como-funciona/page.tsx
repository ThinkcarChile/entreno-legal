...
const clientFlow = [
  ...
  "Eliges una oferta y pagas. El dinero queda asociado a ese trabajo.",
...
export default function HowItWorksPage() {
  return (
    <>
      ...
      <Section title="Si necesitas ayuda" className="bg-surface">
        <ol className="grid gap-4 sm:grid-cols-2">
          {clientFlow.map((item, index) => (
            <li
              key={item}
              className="flex gap-3.5 rounded-[var(--radius-card)] border border-line bg-surface p-5"
            >
              <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-50 text-small font-semibold text-brand-700">
                {index + 1}
              </span>
      ...
      <Section title="Extensiones y disputas" className="bg-surface">
        <div className="grid gap-5 sm:grid-cols-2">
          <article className="rounded-[var(--radius-card)] border border-line bg-surface p-6">
            <h3 className="font-semibold text-ink-950">Si el trabajo se alarga</h3>
