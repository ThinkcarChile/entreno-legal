...
export function PriceHint({ suggestion }: { suggestion: PriceSuggestion }) {
  return (
    <div className="rounded-[var(--radius-card)] border border-brand-100 bg-brand-50/60 p-5">
      <p className="flex items-center gap-2 text-small font-medium text-brand-800">
      ...
      {suggestion.factors.length > 0 && (
        <ul className="mt-4 flex flex-wrap gap-2">
          {suggestion.factors.map((factor) => (
            <li
              key={factor.code}
              className="rounded-full bg-surface px-2.5 py-1 text-caption text-ink-600 ring-1 ring-brand-100 ring-inset"
            >
              {factor.label}
            </li>
          ))}
