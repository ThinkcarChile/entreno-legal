...
/**
 * Toda la corrida vive acá adentro, y NO en el tope del módulo, por el código
 * de salida.
 *
 * `morir()` lanza `SalidaLimpia` en vez de llamar a `process.exit()`, porque
 * `process.exit()` con un socket de `fetch` todavía abierto revienta en Windows
 * («Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)») y devuelve 127. Pero
 * lanzar desde el tope de un módulo ESM no es mejor: la excepción sale sin que
 * nadie la atrape, Node imprime la pila entera DESPUÉS del mensaje —incluido el
 * más importante del sistema, «EL RESPALDO SE ESCRIBIÓ PERO NO SE PUDO
 * RESTAURAR»— y el código de salida deja de ser el que este script eligió.
 *
 * Con la corrida adentro de una función y el `catch` de abajo, `SalidaLimpia` se
 * convierte en `process.exitCode` y Node cierra sus sockets solo antes de irse.
 * Mismo patrón que `scripts/aplicar-migracion.mjs`.
 */
async function principal() {
  ...
  if (!ENSAYAR) {
    ...
    // `return`, no `process.exit(0)`: acá arriba ya hubo varios `fetch` a la
    // Management API y sus sockets siguen abiertos. Salir a la fuerza hacía
    // reventar libuv en Windows y devolver 127 en el camino de ÉXITO — un
    // respaldo bueno anunciado como falla, justo contra lo que promete el
    // runbook («termina con código distinto de cero cuando algo sale mal»).
    return;
  }
...
try {
  ...
  if (e instanceof SalidaLimpia) {
    ...
    console.error(redactar(e instanceof Error ? (e.stack ?? e.message) : String(e)));
    process.exitCode = 1;
  }
}
